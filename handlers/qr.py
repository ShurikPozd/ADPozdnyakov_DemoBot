...
async def qr_start(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Отправьте текст или ссылку для генерации QR-кода.", reply_markup=get_cancel_kb())
...
async def process_qr(message: types.Message, state: FSMContext) -> None:
    ...
    if not data:
        await message.answer("Пожалуйста, отправьте текст или ссылку.", reply_markup=get_cancel_kb())
        return
    ...
    if len(data) > 500:
        ...
        await message.answer("Текст слишком длинный (макс. 500 символов).\n" \
                             "Попробуйте текст/ссылку короче", reply_markup=get_cancel_kb())
        return
