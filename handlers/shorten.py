...
async def process_shorten(message: types.Message, state: FSMContext) -> None:
    ...
    logger.debug(f"Пользователь {message.from_user.id} отправил запрос на сокращение: {long_url}")
    short = await shorten_url(long_url)
    if short:
        await message.answer(f"Короткая ссылка: {short}")
        logger.info(f"URL {long_url} сокращён для пользователя {message.from_user.id}: {short}")
