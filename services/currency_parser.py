...
def parse_currency_request(
    ...
    if money_spans:
        ...
        if not amount_match:
            ...
            normalized = " ".join(
                [normalize_currency_name(w) for w in currency_part.split()]
            )
            ...
            if lemmatized_currency in currency_names:
                ...
                first_word = (
                    lemmatized_currency.split()[0]
                    if lemmatized_currency.split()
                    else ""
                )
            ...
            if not from_currency:
                ...
                normalized_remaining = " ".join(
                    [normalize_currency_name(w) for w in remaining.split()]
                )
                ...
                for name in sorted_names:
                    if (
                        name in lemmatized_remaining
                        and currency_names[name] != from_currency
                    ):
    ...
    if len(found) < 2:
        logger.debug(
            f"Fallback: found only {len(set(c for _, c in found))} unique currencies"
        )
        return None
